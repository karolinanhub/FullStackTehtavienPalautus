...
loginRouter.post('/', async (request, response) => {
  ...
  const user = await User.findOne({ username }) //haetaan käyttäjä tietokannasta
  const passwordCorrect = user === null
    ? false
    : await bcrypt.compare(password, user.passwordHash) //verrataan salasanaa ja hashattua salasanaa
